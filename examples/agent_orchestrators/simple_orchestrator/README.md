...
agents from the [agents examples](../../agents/), or any other agents that you have set up.

## Setting Up the Orchestrator

1. On the `Agents` pane, click **Create an Agent Orchestrator**.
2. Create a new experiment. Everything can be left as the default settings except the `Agents` tab.
3. **Agents:** Add any sub-agents that you would like (at least two are recommended).
4. Click `Create`.
5. Start the orchestrator's server.

## How It Works

The no-code orchestrator uses RAM's built-in orchestrator agent, which does the following:
...
orchestration logic (such as custom routing, multi-step research, and specialized prompts), see the
[News Orchestrator](../news_orchestrator/) example that uses a code template.
